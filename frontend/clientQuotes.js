...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    ...
    list.innerHTML = rows.map((r) => `
      <li>
        <button class="row" data-id="${r.quoteID}" data-data="${encodeURIComponent(JSON.stringify(r))}">
          ${r.client_name}
        </button>
      </li>
    `
  ...
  list.addEventListener("click", (e) => {
    ...
    const minutes = String(dateObj.getMinutes()).padStart(2, '0');
      ...
      document.getElementById("d-date").value = `${year}-${month}-${day}T${hours}:${minutes}`;
      document.getElementById("d-budget").value = r.price || 0.00;
